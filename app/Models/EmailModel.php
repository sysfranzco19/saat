...
class EmailModel extends Model
{
    ...
    function recojo_auth_email($student = '', $detalle = '', $solicitante = '', $fecha = '', $obs = '')
	{
        $msg = '';
        $msg .= $this->header();
        $msg .= '<p style="margin:0;">Estudiante: '.$student.'</p>';
        $msg .= '<p style="margin:0;">Fecha: '.$fecha.'</p>';
        $msg .= '<p style="margin:0;">Cambio: '.$detalle.'</p>';
        $msg .= '<p style="margin:0;">Solicitado por: '.$solicitante.'</p>';
        if ($obs !== '') $msg .= '<p style="margin:0;">Nota: '.$obs.'</p>';
        $msg .= '<p style="margin:10px 0 0;font-weight:bold;color:#50cd89;">✓ Este aviso ha sido APROBADO.</p>';
        $msg .= $this->footer();
        $msg = str_replace("@title@", "Cambio de Recojo Aprobado", $msg);
        return $msg;
    }
    function recojo_noauth_email($student = '', $detalle = '', $solicitante = '', $fecha = '', $obs = '')
	{
        $msg = '';
        $msg .= $this->header();
        $msg .= '<p style="margin:0;">Estudiante: '.$student.'</p>';
        ...
        $msg .= '<p style="margin:0;">Cambio: '.$detalle.'</p>';
        $msg .= '<p style="margin:0;">Solicitado por: '.$solicitante.'</p>';
        if ($obs !== '') $msg .= '<p style="margin:0;">Motivo del rechazo: '.$obs.'</p>';
        $msg .= '<p style="margin:10px 0 0;font-weight:bold;color:#f1416c;">✗ Este aviso NO ha sido aprobado. Por favor, contacte con Secretaría de Nivel.</p>';
        $msg .= $this->footer();
        $msg = str_replace("@title@", "Cambio de Recojo No Aprobado", $msg);
        return $msg;
    }
    ...
    function alerta_cupo6_email($student = '', $dias = 0, $nick_name = '')
    {
        $msg = '';
        $msg .= $this->header();
        $msg .= '<p style="margin:0;">Estudiante: '.$student.($nick_name ? ' — '.$nick_name : '').'</p>';
        $msg .= '<p style="margin:0;">Días de licencia consumidos este trimestre: '.$dias.' / 9</p>';
        $msg .= '<p style="margin:0;">Corresponde coordinar una reunión con la familia.</p>';
        $msg .= $this->footer();
        $msg = str_replace("@title@", "Alerta de Cupo Trimestral — 6 días", $msg);
        return $msg;
    }

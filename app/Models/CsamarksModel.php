...
class CsamarksModel extends Model
{
    ...
    public function saber_hacer_section($section_id, $phase_id){
        ...
        public function student_csamarks($student_id, $phase_id)
    {
        $sql = "SELECT
                    n.csamarks_id, CONCAT(e.lastname,' ', e.lastname2, ' ', e.name) as student, s.name as materia, 
                    n.ser_average, n.saber_average, n.hacer_average, n.autoevaluacion, n.total_average, n.total_vc, n.student_id, n.phase_id
                FROM
                    csamarks n
                INNER JOIN subject s ON
                    (n.subject_id = s.subject_id)
                INNER JOIN t_student e ON
                    (n.student_id=e.student_id) 
                WHERE
                    n.phase_id = ".$phase_id." AND n.student_id=".$student_id."
                ORDER BY
                    n.csamarks_id;";
        $subject = $this->db->query($sql);
        return $subject->getResultArray();
    }
    public function csamarks($csamarks_id)
    {
        $sql = "SELECT
                    n.csamarks_id, CONCAT(e.lastname,' ', e.lastname2, ' ', e.name) as student, s.name as materia, 
                    n.ser_average, n.saber_average, n.hacer_average, n.autoevaluacion, n.total_average, n.total_vc, n.student_id, n.phase_id
                ...
                INNER JOIN t_student e ON
                    (n.student_id=e.student_id) 
                WHERE
                    n.csamarks_id=".$csamarks_id."
                ORDER BY
                    n.csamarks_id;";
        $subject = $this->db->query($sql);
        return $subject->getResultArray();
    }
    public function csamarks_bth_especialidad($student_id, $phase_id){
        $sql = "SELECT c.student_id, s.name, s.hours, c.total_average
                FROM csamarks c INNER JOIN subject s ON(c.subject_id=s.subject_id)
                WHERE c.student_id=".$student_id." AND c.phase_id=".$phase_id." AND s.name LIKE 'BTH%' ORDER BY s.subject_id;";
        $csamarks = $this->db->query($sql);
        return $csamarks->getResultArray();
    }
}

...
function renderTable(data) {
    ...
    const rows = data.map(row => {
        ...
        return `<tr class="${rowCls}">
            ...
            <td class="text-center">
                <a href="${BASE_URL}manager/prim_licencias?nueva_licencia=1&student_id=${row.student_id}"
...
function cargar() {
    ...
    fetch(BASE_URL + 'manager/prim_ausencias_data', {
        method: 'POST',
        headers: {
...
function exportarExcel() {
    ...
    window.location.href = BASE_URL + 'manager/prim_ausencias_xlsx?' + params.toString();
}

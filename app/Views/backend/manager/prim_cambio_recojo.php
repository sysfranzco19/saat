...
function accionesHtml(row) {
    ...
    if (row.enviado != 2) {
        html += `<button class="btn-rechazar" onclick="accion(${row.cambio_id},2)">✗ Rechazar</button>`;
    }
    return html;
}
...
function cargar() {
    ...
    post(BASE + 'manager/prim_cambio_recojo_data', {
...
document.getElementById('modalAccionConfirm').addEventListener('click', function() {
    ...
    const url = tipo === 1
        ? BASE + 'manager/prim_cambio_recojo_auth'
        : BASE + 'manager/prim_cambio_recojo_noauth';
...
document.getElementById('form_rec').addEventListener('submit', function(e) {
    ...
    post(BASE + 'manager/prim_cambio_recojo_create', {

...
function guardarAsistenciaDia() {
    ...
    fetch('<?= base_url('manager/prim_asistencia_save_bulk') ?>', {
        method: 'POST',
        headers: {
...
function cargar() {
    ...
    if (MODE === 'dia') {
        ...
        url  = '<?= base_url('manager/prim_asistencia_data') ?>';
        body = 'date=' + encodeURIComponent(date);
    } else if (MODE === 'mes') {
        const r = computeRange();
        url  = '<?= base_url('manager/prim_asistencia_mes_data') ?>';
        body = 'fecha_ini=' + encodeURIComponent(r.ini) + '&fecha_fin=' + encodeURIComponent(r.fin);
    } else {
        const r = computeRange();
        url  = '<?= base_url('manager/prim_asistencia_resumen') ?>';
        body = 'fecha_ini=' + encodeURIComponent(r.ini) + '&fecha_fin=' + encodeURIComponent(r.fin);
    }
